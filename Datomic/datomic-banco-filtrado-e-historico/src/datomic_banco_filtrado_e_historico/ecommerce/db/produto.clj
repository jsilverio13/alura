(ns datomic-banco-filtrado-e-historico.ecommerce.db.produto
  (:require [datomic-banco-filtrado-e-historico.ecommerce.db.entidade :as db.entidade]
            [datomic-banco-filtrado-e-historico.ecommerce.model :as model]
            [datomic.api :as d]
            [schema.core :as s]
            [clojure.set :as cset]))
...
(s/defn atualiza! [conn , antigo :- model/Produto, a-atualizar :- model/Produto]
  (let [produto-id (:produto/id antigo)
        ...
        txs (map (fn [atributo]
                   [:db/cas [:produto/id produto-id] atributo
                    (get antigo atributo)
                    (get a-atualizar atributo)]) atributos)]
    (d/transact conn txs)))

(defn total [conn]
  (d/q '[:find [(count ?produto)]
         :where [?produto :produto/nome]] conn))
