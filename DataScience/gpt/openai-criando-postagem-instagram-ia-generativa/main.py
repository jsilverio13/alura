...
def selecionar_imagem(lista_nome_imagens):
    opcao = int(input("Qual imagem você deseja selecionar, informe o numero do sufixo da imagem gerada? "))
    if opcao < 0 or opcao > 3:
        opcao = 0
    return lista_nome_imagens[opcao]


def ferramenta_converte_string(dado):
    if isinstance(dado, bytes):
        return str(dado.decode())
    return dado
...
def main():
    ...
    lista_nome_imagens = ferramenta_download_imagem(nome_arquivo,imagem_gerada, qtd_imagens)
    
    caminho_imagem_escolhida = selecionar_imagem(lista_nome_imagens)
    caminho_imagem_convertida = ferramenta_converter_png_para_jpg(caminho_imagem_escolhida, nome_arquivo)
    
    if confirmar_postagem(caminho_imagem_convertida, "Link do Podcast: " + url_podcast + " \n" + ferramenta_converte_string(resumo_instagram) + "\n" + ferramenta_converte_string(hashtags)).lower() == "s":
            postar_instagram(caminho_imagem_convertida, 
                        "Link do Podcast: " + url_podcast + " \n" + ferramenta_converte_string(resumo_instagram) + "\n" + ferramenta_converte_string(hashtags), 
                        usuario_instagram, 
                        senha_instagram)
