  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        ...
        decoration: BoxDecoration(border: Border.all(width: 8), color: Colors.white),
        child: Form(
          child: Center(
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  const Text("por Alura", style: TextStyle(fontStyle: FontStyle.italic)),
                  const Padding(
    ...
    service.login(email: email, password: password).then(
      ...
      (error) {
        showConfirmationDialog(context,
                content: "Deseja criar um novo usuário usando o e-mail $email? e a senha inserida?",
                affirmativeOption: "CRIAR")
            .then((value) async {
          if (value != null && value == true) {
            await service.register(email: email, password: password).then((resultRegister) {
              if (resultRegister) {
                Navigator.pushReplacementNamed(context, "home");
              }
            });
